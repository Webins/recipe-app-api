...
def sample_ingredient(user, name='default tag'):
    """Create and return a sample tag """
...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_partial_update_recipe(self):
        ...
        payload = {'title': 'Chicken Tikkan', 'tags': [new_tag.id]}
    ...
    def test_full_update_recipe(self):
        ...
        payload = {'title': 'Spaguetti carbonara',
                   'time_minutes': 25,
                   'price': 5.00
                   }
        url = detail_url(recipe.id)
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_filter_recipes_by_ingredients(self):
        ...
        recipe1 = sample_recipe(user=self.user, title='recipe1')
        recipe2 = sample_recipe(user=self.user, title='recipe2')
        ig1 = sample_ingredient(user=self.user, name='ing1')
        ig2 = sample_ingredient(user=self.user, name='ing2')
        recipe1.ingredients.add(ig1)
        recipe2.ingredients.add(ig2)
        recipe3 = sample_recipe(user=self.user, title='recipe3')

        res = self.client.get(
            RECIPES_URL, {'ingredients': f'{ig1.id},{ig2.id}'})
